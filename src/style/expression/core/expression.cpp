...
void Expression::evaluate(const Input &input)
{
  ...
  index = 0;
  registers = Kokkos::View<double**,Kokkos::SharedSpace>("expression", registers.extent(0), 1);
...
void Expression::evaluate(Solid &solid)
{
  ...
  index = 0;
  registers = Kokkos::View<double**,Kokkos::SharedSpace>("expression", registers.extent(0), solid.np_local);
...
void Expression::evaluate(Grid &grid)
{
  ...
  index = 0;
  registers = Kokkos::View<double**,Kokkos::SharedSpace>("expression", registers.extent(0), grid.nnodes);
