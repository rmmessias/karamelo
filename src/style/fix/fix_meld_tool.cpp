...
void FixMeldTool::initial_integrate(Solid &solid)
{
  ...
  Kokkos::parallel_reduce("FixMeldTool::initial_integrate", solid.np_local,
			  KOKKOS_LAMBDA(const int &ip, double &ftot0, double &ftot1, double &ftot2)
  {
    if (!mass[ip] || !(mask[ip] & groupbit))
      return;

    const double &c = Kokkos::cos(theta_(0, ip));
    const double &s = Kokkos::sin(theta_(0, ip));
    ...
    xprime = R*xprime;
    const double &pext = Rmax - Kokkos::sqrt(rSq);
